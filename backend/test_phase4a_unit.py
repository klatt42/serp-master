"""
Test Phase 4A: Competitor Analyzer (Unit Tests with Mocks)
Tests the CompetitorAnalyzer logic without making real API calls
"""
...
async def test_competitor_analyzer():
    ...
    print("=" * 60)
    print("Phase 4A: Competitor Analyzer Unit Test")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n[1/6] Testing class initialization...")
        ...
        # Test audit_site with mock
        print(f"\n[2/6] Testing single site audit (mocked)...")

        # Mock the crawler.crawl_site method
        mock_crawl_result = {
            "summary": {
                "pages_crawled": 10,
                "pages_in_queue": 0,
            ...
            "pages": [],
            "issues": {"technical": [], "onpage": [], "structure": []},
            "metadata": {"task_id": "test-123"}
        }

        with patch.object(analyzer.crawler, 'crawl_site', new=AsyncMock(return_value=mock_crawl_result)):
            ...
            assert single_audit["status"] == "complete", "Audit should complete successfully"
        ...
        # Test multiple site audits (parallel) with mock
        print(f"\n[3/6] Testing parallel multi-site audits (mocked)...")
        ...
        with patch.object(analyzer.crawler, 'crawl_site', new=AsyncMock(return_value=mock_crawl_result)):
            ...
            for audit in multi_audits:
                print(f"    - {audit.get('url')}: {audit.get('status')} (score: {audit.get('total_score', 0)})")

        # Test score comparison
        print(f"\n[4/6] Testing score comparison...")
        ...
        assert comparison["user_rank"] == 2, f"Expected user_rank=2, got {comparison['user_rank']}"
        print(f"  ✓ Comparison calculated")
        print(f"    User rank: {comparison.get('user_rank')} / {comparison.get('total_sites')}")
        print(f"    Score gap to first: {comparison.get('score_gap_to_first')} pts")

        # Test gap calculation
        print(f"\n[5/6] Testing gap calculation...")
        user_with_scores = {
            "url": "user.com",
            "total_score": 40,
            "scores": {
        ...
        print(f"\n[6/6] Testing competitive strategy generation...")
        strategy = analyzer.generate_competitive_strategy(gaps, user_with_scores, comp_with_scores)
        assert isinstance(strategy, list), "Strategy should be a list"
        print(f"  ✓ Strategy generated: {len(strategy)} actions")
        for action in strategy[:3]:  # Show first 3
            print(f"    - {action.get('action')}: {action.get('impact')} pts impact")

        print("\n" + "=" * 60)
        print("✅ Phase 4A: ALL UNIT TESTS PASSED")
        print("=" * 60)
        print("\nCompetitor Analyzer logic is working correctly!")
        print("✓ No coroutine warnings")
        print("✓ Async/await properly implemented")
        print("✓ All methods functional")
        print("\nReady to proceed to Phase 4B: Comparison API Endpoints\n")
